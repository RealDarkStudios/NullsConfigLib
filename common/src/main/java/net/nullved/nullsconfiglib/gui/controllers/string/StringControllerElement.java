...
import net.nullved.nullsconfiglib.gui.utils.UndoRedoHelper;
...
public class StringControllerElement extends ControllerWidget<IStringController<?>> {
    ...
    public StringControllerElement(IStringController<?> controller, NCLScreen screen, Dimension<Integer> widgetDimension, boolean instantApply) {
        super(controller, screen, widgetDimension);
        this.instantApply = instantApply;
        inputField = controller.getString();
        inputFieldFocused = false;
        selectionLength = 0;
        emptyText = Component.literal("Click to type...").withStyle(ChatFormatting.GRAY);
        controller.option().addListener((opt, val) -> {
            inputField = controller.getString();
        });
...
//            }
    ...
    public boolean modifyInput(Consumer<StringBuilder> consumer) {
        ...
        if (!controller.isInputValid(temp.toString()))
            return false;
...
//        undoRedoHelper.save(inputField, caretPos, selectionLength);
    ...
    protected void updateControl() {
        controller.setFromString(inputField);
    }
    ...
    protected Component getValueText() {
        ...
        return instantApply || !inputFieldFocused ? controller.formatValue() : Component.literal(inputField);
    }
}

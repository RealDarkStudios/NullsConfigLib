...
public class ColorController implements IStringController<Color> {
    private final IOption<Color> option;
    private final boolean allowAlpha;

    public ColorController(IOption<Color> option) {
        this(option, false);
    }

    public ColorController(IOption<Color> option, boolean allowAlpha) {
        this.option = option;
        this.allowAlpha = allowAlpha;
    }

    @Override
    public IOption<Color> option() {
        return option;
    }
    ...
    public static class ColorControllerElement extends StringControllerElement {
        ...
        public ColorControllerElement(ColorController control, NCLScreen screen, Dimension<Integer> dim) {
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            if (super.mouseClicked(mouseX, mouseY, button)) {
                ...
                if(isMouseOverColorPreview(mouseX, mouseY)) {
                    ...
                    if(NCLConfig.HANDLER.instance().showColorPickerIndicator) {
                        NCLConfig.HANDLER.instance().showColorPickerIndicator = false;
                        NCLConfig.HANDLER.save();
                    }
                }
                caretPos = Math.max(1, caretPos);
                setSelectionLength();
        ...
        public Color getPreviewOutlineColor(boolean colorPreviewHovered) {
            ...
            } else if(NCLConfig.HANDLER.instance().showColorPickerIndicator) {
